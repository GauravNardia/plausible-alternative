...
const Page = async ({ params }: Params) => {
  ...
  // const { site, count: sitesUsed } = await getSiteAndCountByUserId(userId)
    const [usageRes, sitesData] = await Promise.all([
    fetch(`${process.env.NEXT_PUBLIC_APP_URL!}/api/usage?userId=${userId}`, { cache: "no-store" }).then(r => r.json()),
    getSiteAndCountByUserId(userId)
  ])
